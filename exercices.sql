...
select h.nom  from habitant h where h.num_qualite isnull;
...
select v.nom_village from village v
where v.nb_huttes = (select max(v2.nb_huttes)
					  from village v2);
...
from trophee t2 join habitant h2 on h2.num_hab = t2.num_preneur 
where h2.nom = 'Obélix');
